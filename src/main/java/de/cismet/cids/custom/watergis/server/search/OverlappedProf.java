 ...
 */
public class OverlappedProf extends AbstractAnalyzeSearch {
    ...
    protected String createQuery(final String owner, final int[] ids) {
        if ((owner == null) || owner.startsWith("lung")
            ...
            return
                "select id, geom, ww_gr, ba_cd, ba_st_von, ba_st_bis, laenge, fis_g_date, fis_g_user from dlm25w.select_overlapped_fg_ba_prof('"
                        + owner
                        + "', "
                        + createSqlString(ids)
                        + ")";
