...
public class FgBaDueTrigger extends AbstractDBAwareCidsTrigger {
    ...
    public CidsTriggerKey getTriggerKey() {
        return new CidsTriggerKey(CidsTriggerKey.ALL, FG_BA_DUE_TABLE_NAME);
    }
                ...
                final Thread t = new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                    ...
                                    LOG.error(
                                        "time to update async for fg_ba_due "
                                                + String.valueOf(cidsBean.getMetaObject().getID())
                                                + " :"
                                                + (System.currentTimeMillis() - start));
                                } catch (Exception e) {
                                    LOG.error(
                                        "Error while executing async fg_ba_du trigger."
                                                + String.valueOf(cidsBean.getMetaObject().getID()),
                                        e);
                                } finally {
                                    if (con != null) {
